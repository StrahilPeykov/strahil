    ...
    <>
      ...
      >
        <div className="container max-w-7xl mx-auto px-6">
          <div className={
            `rounded-xl border ${isScrolled ? 'border-border backdrop-blur-xl' : 'border-transparent backdrop-blur-lg'} 
             bg-card/70 px-4 py-3 transition-all duration-300`
          }>
                ...
                {moreItems.length > 0 && (
                  <div className="relative ml-1">
                    <button
                      ...
                      className="relative px-3 py-2 text-foreground/70 hover:text-foreground transition-all flex items-center gap-1 text-sm"
                    >
                      <span>More</span>
                    ...
                    <AnimatePresence>
                      {isMoreMenuOpen && (
                        <motion.div
                          ...
                          className="absolute top-full right-0 mt-3 w-48 origin-top-right z-50 bg-card/80 backdrop-blur-xl border border-border rounded-xl shadow-xl overflow-hidden"
                        >
                          <div className="py-2">
                            {moreItems.map((item) => {
                              ...
                              return (
                                <Link
                                  ...
                                  className={`group flex items-center gap-3 px-4 py-2 text-sm transition-all ${
                                    isActive
                                      ? 'text-foreground bg-glow/10 border-r-2 border-glow'
                                      : 'text-foreground/70 hover:text-foreground hover:bg-muted/50'
                                  }`}
                                >
                                  <Icon size={16} className={isActive ? 'text-foreground' : 'text-foreground/60 group-hover:text-foreground'} />
                                  <div>
